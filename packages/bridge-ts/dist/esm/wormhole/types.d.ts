...
export type WormholeArbitrumContractAddresses = typeof CONTRACTS.MAINNET.arbitrum;
export type WormholeEthereumContractAddresses = typeof CONTRACTS.MAINNET.ethereum;
...
export declare enum WormholeSource {
    Ethereum = "ethereum",
    Arbitrum = "arbitrum",
    Solana = "solana",
    Polygon = "polygon",
...
export interface WormholeClient<T extends (ethers.ContractReceipt & {
    txHash: string;
}) | TxResponse | TransactionResponse, R extends Transaction | MsgExecuteContractCompat | ethers.ContractReceipt> {
    getAddress: () => Promise<string>;
    getBalance: (address: string, tokenAddress?: string) => Promise<string>;
    transfer: (args: TransferMsgArgs) => Promise<T>;
    getSignedVAA: (tx: T) => Promise<string>;
    ...
    getForeignAsset: (originChain: ChainId, originAsset: string) => Promise<string>;
}
