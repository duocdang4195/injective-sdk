...
import { buildTokenId, coalesceChainId, hexToUint8Array, CHAIN_ID_INJECTIVE, isNativeCosmWasmDenom, tryNativeToHexString, } from '@injectivelabs/wormhole-sdk';
 ...
 */
export async function getForeignAssetInjective(tokenBridgeAddress, client, originChain, originAsset) {
    try {
 ...
 */
export async function getIsWrappedAssetInjective(tokenBridgeAddress, client, assetAddress) {
    const hexified = tryNativeToHexString(assetAddress, 'injective');
    const result = await getForeignAssetInjective(tokenBridgeAddress, client, CHAIN_ID_INJECTIVE, new Uint8Array(Buffer.from(hexified)));
    if (result === null) {
        return false;
    }
    return true;
 ...
 * @returns Information about the asset
 */
export async function getOriginalAssetInjective(wrappedAddress, client) {
    ...
    try {
        const response = await client.fetchSmartContractState(wrappedAddress, Buffer.from(JSON.stringify({
            wrapped_asset_info: {},
        })).toString('base64'));
...
export const queryExternalIdInjective = async (client, tokenBridgeAddress, externalTokenId) => {
    try {
        const response = await client.fetchSmartContractState(tokenBridgeAddress, Buffer.from(JSON.stringify({
            external_id: {
                external_id: Buffer.from(externalTokenId, 'hex').toString('base64'),
            },
        })).toString('base64'));
        const parsedResponse = parseSmartContractStateResponse(response);
        const denomOrAddress = parsedResponse.token_id.Bank?.denom ||
