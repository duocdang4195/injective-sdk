import { TxGrpcApi, hexToBuff, PublicKey, SIGN_AMINO, hexToBase64, BaseAccount, ChainRestAuthApi, createTxRawEIP712, createTransaction, getEip712TypedData, createWeb3Extension, ChainRestTendermintApi, createTransactionWithSigners, createTxRawFromSigResponse, IndexerGrpcTransactionApi, getGasPriceBasedOnMessage, recoverTypedSignaturePubKey, } from '@injectivelabs/sdk-ts';
...
import { getEthereumSignerAddress, getInjectiveSignerAddress, } from '../utils/utils';
...
export class MsgBroadcaster {
    ...
    constructor(options) {
        ...
        const endpoints = options.networkEndpoints || getNetworkEndpoints(options.network);
     ...
     * @param tx the transaction that needs to be broadcasted
     */
    async experimentalBroadcastKeplrWithLedger(tx) {
        const { options, endpoints, chainId, ethereumChainId } = this;
        const { walletStrategy } = options;
        const msgs = Array.isArray(tx.msgs) ? tx.msgs : [tx.msgs];
        /**
        ...
        if (walletStrategy.getWallet() === Wallet.Keplr) {
            ...
            const isLedgerConnectedOnKeplr = walletDeviceType === WalletDeviceType.Hardware;
            if (!isLedgerConnectedOnKeplr) {
                throw new GeneralException(new Error('This method can only be used when Keplr is connected with Ledger'));
            }
        }
        if (!ethereumChainId) {
            throw new GeneralException(new Error('Please provide ethereumChainId'));
    ...
    async broadcastCosmosWithFeeDelegation(tx) {
        ...
        const feePayerAccountDetailsResponse = await chainRestAuthApi.fetchAccount(feePayer);
        const feePayerBaseAccount = BaseAccount.fromRestApi(feePayerAccountDetailsResponse);
