...
import { Erc20Contract, PeggyContract, getContractAddressesForNetworkOrThrow, } from '@injectivelabs/contracts';
...
export class Web3Composer {
    ...
    async getSetTokenAllowanceTx({ address, amount, gasPrice, tokenAddress, }) {
        ...
        const gas = new BigNumberInWei(await setAllowanceOfContractFunction.estimateGasAsync());
        return {
            from: address,
            to: tokenAddress,
    ...
    async getPeggyTransferTx({ address, amount, denom, destinationAddress, gasPrice, data = '', }) {
        const { network, ethereumChainId } = this;
        const alchemy = await this.getAlchemy();
        const ethersProvider = await alchemy.config.getProvider();
        const contractAddresses = getContractAddressesForNetworkOrThrow(network);
        const contractAddress = denom === INJ_DENOM
        ...
        const gas = new BigNumberInWei(await depositForContractFunction.estimateGasAsync());
        return {
            from: address,
