...
import { getCosmosExplorerUrl, getPolygonExplorerUrl, getSolanaExplorerUrl, } from './bridge';
import { BridgingNetwork } from '../types/bridge';
import { getEthereumExplorerUrl, getNetworkFromAddress } from './bridge';
export const getSupportedNetworkFromAddress = (address) => {
...
export const getBlockExplorerPathFromNetworkType = ({ network, chain, address, }) => {
    ...
    if (Object.values(BridgingNetwork).includes(chain)) {
        return `${getCosmosExplorerUrl(chain, network)}/account/${address}`;
    }
    return undefined;
};
